...
2. Search in parallel: fight dimensionality with dimensionality! There are a number of different manifestations of this approach:

* Evolution is effectively a highly parallel search algorithm over the space of genomes and organisms, that takes advantage of the massive parallel computation available across all the ecosystems of planet Earth. All biological organisms depend on the results of this evolutionary search process for a major portion of their basic functionality, with _ontogenetic_ learning within an individual's lifetime contributing varying portions of additional functionality across species. Humans of course pride ourselves on being the prime example of ontogentic learning, but this means that we tend to overlook how much we actually get from the evolutionarily-coded circuits. These circuits are generally thought to be concentrated in the subcortical brain systems, with the [[neocortex]] being primarily shaped by learning.

    From a pragmatic perspective in trying to capture the computational capabilities of the human brain, the evolutionary contribution can be represented by [[computational-cognitive-neuroscience#reverse engineering the brain]] to try to understand how the subcortical networks interact with neocortical learning. This is the approach taken in the [[Rubicon]] framework, and in the biologically-based [[Axon]] approach more generally. Alternatively (or in addition), evolutionary search can be simulated using [[genetic algorithms]], but without the advantage of the native parallelism of physical systems, and eons of time at our disposal, this is not likely a very practical approach.

* Cultural evolution and culture-based learning (e.g., the educational system) represent another important form of parallel search, where each individual in a society, across time, searches though different parts of the overall space, and then can share back with everyone else what they have discovered. The many celebrated advances across math, science, and the humanities that we all learn about in school provide an incredibly advanced foundation for further learning, compared to what an individual by themselves "in the state of nature" would be able to discover. Of course, all of this depends critically on [[language]].

* Parallel search through possible solutions / plans via parallel [[constraint satisfaction]] and [[optimized representations]] more generally. In short, if you can use this parallel search to rapidly find "reasonable" options to explore through actual serial behavioral trial-and-error exploration, then the effective search space can be dramatically reduced in size.
