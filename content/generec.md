...
[[#figure_bidir-err]] shows all of the major dynamics that enable bidirectional excitatory connections coupled with a temporal derivative learning mechanism to compute an error gradient. This figure is a more detailed version of the simple Minus phase $\rightarrow$ Plus phase [[temporal derivative#figure_minus-plus]] in the [[temporal derivative]] page.
...
Thus, the Hidden neuron experiences a temporal derivative that reflects the corresponding temporal changes on the Prediction layer, by virtue of its bidirectional connectivity. The mathematical derivation below shows that this temporal derivative provides a good approximation to the error gradient computed by error backpropagation.

In effect, GeneRec uses _activation_ states to compute error derivatives, while backpropagation directly computes an error gradient that is entirely separate from its activation state. As a consequence, GeneRec automatically generates [[optimized representations]] that include the error gradient in the plus phase, which is equivalent to a form of [[error backpropagation#backpropagation to activations]], whereas standard feedforward backpropagation networks do not, as we elaborate below. This is in addition to the existing optimization of the representations via [[constraint satisfaction]], also driven by bidirectional connectivity.

In sum, the GeneRec algorithm shows how bidirectional connectivity, which is a prominent feature of the [[neocortex]], enables powerful error-driven learning in a biologically plausible manner. Various closely related proposals have also been advanced since the original GeneRec paper, providing different analysis and routes of derivation ([[@XieSeung03]]; [[@ScellierBengio17]]; [[@WhittingtonBogacz17]]).
